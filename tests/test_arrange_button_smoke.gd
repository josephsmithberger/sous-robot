...
func _ready() -> void:
	...
	# 2. Verify ArrangeButton exists, is visible, and starts as "ARRANGE"
	...
	assert(arrange_button.text == "ARRANGE", "ArrangeButton text must start as 'ARRANGE'.")
	...
	assert(arrange_button.text == "CONFIRM", "ArrangeButton text must change to 'CONFIRM'.")
	...
	assert(arrange_button.text == "ARRANGE", "ArrangeButton text must return to 'ARRANGE'.")
